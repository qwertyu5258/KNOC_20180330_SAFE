...
%>
			...
			<% } %>
					...
					<div class="btnin">
					 ...
					 <% } %>
					 <% if(article.getUrl()!=null){%>
					 <img src="<%=skinRoot %>/images/btn_move.gif" alt="이동" /></a>
					 <% } %>
			<%
			String flag = "N";
			if(article.getFiles() != null && article.getFiles().size() > 0) {
			...
			<%
				AttachFiles afs = article.getFiles();
					 for(int k = 0 ; k < afs.size(); k++) {
						 AttachFile af = afs.getAttachFile(k);
						 if (af.getBun() != null) {
							if(af.getBun().equals("2") && af.getMimeType().equals("pdf")){%><a target="_parent" href="<%=Property.SERVLET_ROOT%>/Download?num=<%=article.getNo()%>&amp;fno=<%=af.getNo()%>&amp;bid=<%=propBean.getBoardID()%>&amp;callback=<%=pageBean.getPageUrl()%>&amp;ses=<%=sessionName%>"><img src="<%=skinRoot %>/images/btn_pdf.gif" alt="PDF다운받기" /></a><%}%>
					<%
