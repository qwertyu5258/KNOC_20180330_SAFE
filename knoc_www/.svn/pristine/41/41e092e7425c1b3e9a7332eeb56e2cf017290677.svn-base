...
function showCalendar(ctl, ctl2, format) {
	var leftpos = 0;
	var toppos = 0;
	...
	if(bPageLoaded) {
		...
		aFormat	= dateFormat.split(formatChar);
			...
			//달경의 위치 선택시 offsetLeft,offsetTop에서 Excepttion이 발생할 경우의 
			//위치 셋팅의 로직을 변경한다.
			try{
				do {
			...
			//위에서 Excetpion 발견시
			if(!isException){
				//Exception 발견시 클릭 위치를 이용한 로직으로 변경
				...
				if(browser=="Microsoft Internet Explorer"){
					...
					//xPoint = event.clientX -300;
					//yPoint = event.clientY - document.documentElement.scrollTop;
				...
				//이거는 그냥 덤으로다가 현재 브라우저의 가운데 좌표 표시
