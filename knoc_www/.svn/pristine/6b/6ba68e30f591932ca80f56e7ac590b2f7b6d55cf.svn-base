...
<% } %>
	...
	<colgroup><col width="100px" /><col width="260px" /><col width="100px" /><col width="260px" /></colgroup>
		...
		<tr>
			<th class="th1">부제</th>
			<td class="td1" colspan="3"><%=article.getContent()%></td>
		</tr>
        <tr>
			<th class="th1">성명</th>
			<td class="td1"><%=article.getName()%></td>
			<th class="th1">등록일</th>
			<td class="td1"><%=article.getRegDateString("yyyy-MM-dd")%></span></td>
		</tr>
		<tr>
			<th class="th1">링크URL</th>
			<td class="td1" colspan="3"><%=article.getUrl()%></td>
		</tr>
		<tr>
...
<% 	if ( article.getFilecnt() > 0 ) {
		...
		for(int k = 0 ; k < afs.size(); k++) {
			...
			if (af.getBun().equals("1")) {
%>
					이미지 업로드 : <a href="<%=Property.SERVLET_ROOT%>/Download?num=<%=pageBean.getNum()%>&fno=<%=af.getNo()%>&bid=<%=propBean.getBoardID()%>&callback=<%=pageBean.getPageUrl()%>&ses=<%=sessionName%>"><img src="<%=af.makeMimeIcon(skinRoot + "/images")%>" width="16" height="16" border=0 alt="<%=af.getFileName()%> : <%=af.getFileSize("HB")%>" align=absmiddle> <%=af.getFileName()%> : <%=af.getFileSize("HB")%></a><br>
<%
			} else if (af.getBun().equals("2")) {
%>
					PDF파일 업로드 : <a href="<%=Property.SERVLET_ROOT%>/Download?num=<%=pageBean.getNum()%>&fno=<%=af.getNo()%>&bid=<%=propBean.getBoardID()%>&callback=<%=pageBean.getPageUrl()%>&ses=<%=sessionName%>"><img src="<%=af.makeMimeIcon(skinRoot + "/images")%>" width="16" height="16" border=0 alt="<%=af.getFileName()%> : <%=af.getFileSize("HB")%>" align=absmiddle> <%=af.getFileName()%> : <%=af.getFileSize("HB")%></a><br>
<%
			}
		}
	} else { %>
