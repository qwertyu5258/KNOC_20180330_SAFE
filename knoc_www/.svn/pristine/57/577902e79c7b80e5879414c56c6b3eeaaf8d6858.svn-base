...
<input name="secret" type="hidden" value="1" />
	...
	<colgroup><col width="110px" /><col width="130px" /><col width="110px" /><col width="130px" /><col width="110px" /><col width="130px" /></colgroup>
		...
		<tr>
			...
			<td class="td1" colspan="5"><input type="text" value="<%=zipno1 %>" id="zipno1" name="zipno1" class="input" style="width:60px;" maxlength="3" readonly onclick="openZipcode();" title="우편번호 앞자리"/> <a href="#" onclick="openZipcode();"><img src="<%=skinRoot %>/images/btn_se.gif" align="top" alt="주소찾기" /></a></td>
		</tr>
		<tr>
	         <td class="td1" colspan="5"><input type="text" value="<%=article.getAddr1() %>" id="addr1" name="addr1" maxlength="200" class="input" style="width:580px;" readonly onclick="openZipcode();"  title="기본 주소"/></td>
		</tr>
	    <tr>
		...
		<tr>
			...
			<td class="td1" colspan="5">
			<%if("사이버민원".equals(article.getContGubun()) || "부조리신고".equals(article.getContGubun())){ %>
				<label for="contGubun1">사이버민원</label>
				<input type="radio" id="contGubun1" name="contGubun" value="사이버민원" <% if (article.getContGubun().equals("사이버민원")) { %>checked="checked"<% } %> />
				<label for="contGubun2">부조리신고</label>
				<input type="radio" id="contGubun2" name="contGubun" value="부조리신고" <% if (article.getContGubun().equals("부조리신고")) { %>checked="checked"<% } %> />
			<%}else if("불법하도급 신고".equals(article.getContGubun())){ %>
				불법하도급 신고
				<input type="hidden" name="contGubun" value="불법하도급 신고" />
				
			<%}else if("공익신고".equals(article.getContGubun())){ %>
				공익신고
		...
		<tr>
			...
			<td class="td1" colspan="5"><textarea name="HTML_BODY" id="HTML_BODY"  cols="95" rows="15" class="area" title="글내용 입력"><%=article.getContent() %></textarea></td>
		</tr>
		<% if(propBean.getUseNotice().equals("1") && Property.isViewable("notice",authBean,propBean,userSession)) { %>
		<tr>
			<th scope="row" class="th1"><label for="notice">공지글</label></th>
			<td class="td1" colspan="5">
...
%>						
	...
	<div class="boardbtm">
		<div class="btnsec">
			...
			<a href="<%=pageBean.getPageUrl()%><%=pageBean.makeQueryString("list",true)%><%=schBean.makeQueryString("","bid=" + propBean.getBoardID()+"&amp;ses="+sessionName+"&amp;pass="+request.getParameter("passwd")+"&amp;name="+article.getName()+"&amp;bno="+article.getNo(),false)%>"><img src="<%=skinRoot %>/images/btn_cancel.gif" alt="취소" /></a>
		</div>
	</div>
</div>
