...
<% } %>		
	...
	<colgroup><col width="100px" /><col width="260px" /><col width="100px" /><col width="260px" /></colgroup>
		<tr>
			...
			<td class="td1"><%=StringUtil.replaceInString(article.getSubject(),"[Re]","<span class=font_12px_20_bold>[Re]</span>")%></td>
			<th class="th2">View</th>
...
<%=StringUtil.replaceInString(article.getContent(),"[Re]","<span class=font_12px_20_bold>[Re]</span>")%>
...
%>		
		...
		<colgroup><col width="95px" /><col width="605px" /></colgroup>
			<tr>
				<th><img src="<%=skinRoot %>/images/ico_prevtxt.gif" alt="" /> Prev</th>
				<td><% if ( prevArticle != null) { %><a href="<%=pageBean.getPageUrl()%><%=pageBean.makeQueryString(prevArticle.getNo(),"view",true)%><%=schBean.makeQueryString("","bid=" + propBean.getBoardID()+"&ses="+sessionName,false)%>"><%=prevArticle.getSubject()%></a><% } else { %>No Prev<% } %></td>
			</tr>
			<tr>
				<th><img src="<%=skinRoot %>/images/ico_nexttxt.gif" alt="" /> Next</th>
				<td><% if ( nextArticle != null) { %><a href="<%=pageBean.getPageUrl()%><%=pageBean.makeQueryString(nextArticle.getNo(),"view",true)%><%=schBean.makeQueryString("","bid=" + propBean.getBoardID()+"&ses="+sessionName,false)%>"><%=nextArticle.getSubject()%></a><% } else { %>No Next<% } %></td>
			</tr>
		</table>
		<!--// prev/next list: end //-->
