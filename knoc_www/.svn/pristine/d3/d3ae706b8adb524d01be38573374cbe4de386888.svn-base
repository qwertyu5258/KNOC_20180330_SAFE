...
public class StringUtil {
	...
	public static String getCurrentDate(){
		SimpleDateFormat df = new SimpleDateFormat("yyyyMMssHHmmss");
		return df.format(Calendar.getInstance().getTime());
	}
	
	public static String getDateType(String date, String format) {
		if (date != null && !date.equals("") && date.length() == 8) {
			date = date.substring(0,4) + "" + format + "" + date.substring(4,6) + "" + format + "" + date.substring(6,8);
		}
		return date;
	}
}