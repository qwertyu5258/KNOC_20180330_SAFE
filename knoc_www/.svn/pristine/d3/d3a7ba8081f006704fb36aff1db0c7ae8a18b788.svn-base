...
<%
	...
	int count = TermMa.getTermListCount();
...
<td valign=top>
	...
	<td valign="top">
		...
		<td valign="top">
			...
			<tr>
			<td><img src="/SiteControl/images/admin_left_03.gif" width="11" height="11"> 기타 관리 > 용어사전 관리</td>
			...
			<input type=button value=' 용어등록  ' class=button onClick="jsWrite();">
			</td>
			</tr>
			</table>
			<br>
...
<%
		...
		while(iter.hasNext()){
			TermBean pb = (TermBean)iter.next();
%>
			<tr height="25"  bgcolor="#FFFFFF">
			<td align=center><%=seq %></td>
			<td align=center><a href="javascript:edit('<%= pb.getNo() %>')"><%= pb.getKorean() %></a></td>
			<td align=center><a href="javascript:edit('<%= pb.getNo() %>')"><%= pb.getEnglish() %></a></td>
			<td align=center><a href="javascript:edit('<%= pb.getNo() %>')"><%= pb.getChinese() %></a></td>
			</tr>
<%
			seq--;
		}
...
%>
		...
		<br>
<stl:pager pageName="pp" prevPage="prevPage" nextPage="nextPage" prevBlock="prevBlock" nextBlock="nextBlock" pages="pages" firstPage="firstPage" lastPage="lastPage" blockSize="10" totalCount="<%=TermMa.getTermListCount()%>" />
