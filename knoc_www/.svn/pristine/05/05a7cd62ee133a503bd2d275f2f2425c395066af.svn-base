...
<%	
	...
	if (depth2.equals("00")) {
		...
		result = MenuMa.updateListnum(mno, listnum1, listnum2, mcode, "DOWN");	
   	} else if (depth3.equals("00")) {
   		mb = MenuMa.getMenu(mno2);
		listnum1 = mb.getListnum2();
		mb = MenuMa.getMenu(mno);		
		listnum2 = mb.getListnum2();
		result = MenuMa.updateListnum2(mno, listnum1, listnum2, mcode, "DOWN");
   	} else {
   		mb = MenuMa.getMenu(mno2);
		listnum1 = mb.getListnum3();
		mb = MenuMa.getMenu(mno);
		listnum2 = mb.getListnum3();
		result = MenuMa.updateListnum3(mno, listnum1, listnum2, mcode, "DOWN");
   	}
...
<script type="text/javascript">
	alert("메뉴수정이 완료되었습니다.");
	location.href = "engMenuListEdit.jsp?depth1=<%=depth1%>&depth2=<%=depth2%>&depth3=<%=depth3%>&depth4=<%=depth4%>";
</script>
<% 	
  	}
%>