...
%>	
		<tr>
			...
			<img src="<%=skinRoot %>/images/btn_no.gif" alt="공지" class="ic"/>
			<% } else { %>
			<%=articleN.getSeq()%>
			<% } %>
			</td>
			...
			<% if (articleN.getIsDelete().equals("Y")) { %>
				<img src="<%=skinRoot %>/images/btn_reple_del.gif" alt="삭제" tabindex="0">
			<% } else { %>
				<!-- 비공개 여부 표시 -->
			...
			<% } %>
			<!-- 답글 공백 깊이 -->
			<%=articleN.makeReply("<img src='"+skinRoot+"/images/icon_reply.gif' alt='답변' class='re'/>","&nbsp;&nbsp;")%>
			<!-- 링크  시작 -->
...
%>	
		<tr>
			...
			<% } %>
			<!-- 답글 공백 깊이 -->
			<%=article.makeReply("<img src='"+skinRoot+"/images/btn_re.gif' alt='답변' class='re' align='absmiddle' />","&nbsp;&nbsp;")%>
			<!-- 링크  시작 -->
