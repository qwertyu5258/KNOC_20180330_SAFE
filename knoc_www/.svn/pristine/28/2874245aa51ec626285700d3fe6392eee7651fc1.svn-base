...
<%	
	...
	PopupNewBean pnb = PopupMa.getPopupNew(no);
%>
...
<script type="text/javascript">
	...
	function jsDelete() {
	    if (confirm("삭제를 하시겠습니까?")) {	
	    	location.href = "popupNewDeleteOk.jsp?no=<%=no%>&nowpage=<%=nowpage%>";	    		
	    }
	}
</script>
...
<form name="wForm" method="post" action="popupNewEditOk.jsp">
<INPUT TYPE="hidden" name="page" value="<%=nowpage%>">
<INPUT TYPE="hidden" name="no" value="<%=no%>">
		<tr>
		<td width="100px" height="25" bgcolor="E7E7E7" align="center">게시기한</td>
		<td bgcolor="#FFFFFF" style="padding-left:10;">
		<input name="StartDate" type="text" size=15 value="<%=StringUtil.getDateType(pnb.getStartDate(),"-")%>" readonly="readonly" class="input date">
		~ <input name="EndDate" type="text" size=15 value="<%=StringUtil.getDateType(pnb.getEndDate(),"-")%>" readonly="readonly" class="input date">
		</td>
		</tr>
		<%
		/**
		...
		<td bgcolor="#FFFFFF" style="padding-left:10;"><input type="radio" name="visibility" value="Y" <% if (etcutil.checkNull(pnb.getVisibility()).equals("Y")) { %>checked<% } %>>표출<input type="radio" name="visibility" value="N" <% if (etcutil.checkNull(pnb.getVisibility()).equals("N")) { %>checked<% } %>>표출안함</td>
		</tr>			
		<tr>
		<td width="100px" height="25" bgcolor="E7E7E7" align="center">제목</td>
		<td bgcolor="#FFFFFF" style="padding-left:10;"><input type="text" name="title" value="<%=pnb.getTitle() %>" maxlength=200 style="width:430px" class=input></td>
		</tr>	
		<tr>
		<td width="100px" height="25" bgcolor="E7E7E7" align="center">내용</td>
		<td bgcolor="#FFFFFF" style="padding-left:10;"><textarea id="content" name="content" cols="85" rows="15" class="area"><%=pnb.getContent() %></textarea></td>
		</tr>
		<tr>
		<td width="100px" height="25" bgcolor="E7E7E7" align="center">가로사이즈</td>
		<td bgcolor="#FFFFFF" style="padding-left:10;"><input type="text" name="width" value="<%=pnb.getWidth() %>" maxlength=5 style="width:130px" class=input></td>
		</tr>
		<tr>
		<td width="100px" height="25" bgcolor="E7E7E7" align="center">세로사이즈</td>
		<td bgcolor="#FFFFFF" style="padding-left:10;"><input type="text" name="height" value="<%=pnb.getHeight() %>" maxlength=5 style="width:130px" class=input> (쿠키 사용할 경우 25px를 더 추가해주세요.)</td>
		</tr>		
		<tr>
		<td width="100px" height="25" bgcolor="E7E7E7" align="center">스크롤 사용여부</td>
		<td bgcolor="#FFFFFF" style="padding-left:10;">
		<input type="radio" name="scroll" value="Y" <% if (etcutil.checkNull(pnb.getScroll()).equals("Y")) { %>checked<% } %> />사용 <input type="radio" name="scroll" value="N" <% if (etcutil.checkNull(pnb.getScroll()).equals("N")) { %>checked<% } %> />미사용
		</td>
		</tr>
		<tr>
		<td width="100px" height="25" bgcolor="E7E7E7" align="center">쿠키 사용여부</td>
		<td bgcolor="#FFFFFF" style="padding-left:10;">
		<input type="radio" name="cookie" value="Y" <% if (etcutil.checkNull(pnb.getCookie()).equals("Y")) { %>checked<% } %> />사용 <input type="radio" name="cookie" value="N" <% if (etcutil.checkNull(pnb.getCookie()).equals("N")) { %>checked<% } %> />미사용
		</td>
		</tr>	
