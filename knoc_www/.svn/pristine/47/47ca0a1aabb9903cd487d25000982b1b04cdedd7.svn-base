...
%>		
	...
	<div class="boardbtm mtm10">
		<div class="btnsec">	
			...
			<% if(Property.isViewable("edit",authBean,propBean,userSession)) { %>
			<a href="edit.jsp"  onclick="PasswdPop('<%=pageBean.getPageUrl()%><%=pageBean.makeQueryString("edit",true)%><%=schBean.makeQueryString("","bid=" + propBean.getBoardID()+"&amp;ses="+sessionName+"&amp;psize="+propBean.getListPerPage(),false)%>'); return false;"><img src="<%=skinRoot %>/images/btn_modi.gif" alt="수정" /></a><% } %>
			<% if(Property.isViewable("delete",authBean,propBean,userSession,article.getId())) { %>
			<a href="#cmtdelete"	onclick="deleteArticle(); return false;"><img src="<%=skinRoot %>/images/btn_del.gif" alt="삭제" /></a><% } %>	    						
