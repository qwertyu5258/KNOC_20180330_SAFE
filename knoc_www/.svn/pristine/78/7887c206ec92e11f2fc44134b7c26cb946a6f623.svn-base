...
<input type="hidden" name="thumbType" value="new2" />
	...
	<colgroup><col width="110px" /><col width="590px" /></colgroup>
		...
		<tr>
			<th class="th1"><label for="subjtxt2">부제</label></th>
		...
		<% } %>
        <tr>
			<th class="th1"><label for="name">성명</label></th>
...
%>		
		<tr>
			<th class="th1"><label for="file1">이미지 업로드</label></th>
			<td class="td1">
				<div class="filedv1">						
<%	 	
...
%>
						<input type="file" name="file1" id="file1" value="" class="input" style="width:330px;" title="첨부파일1" onchange="filterExtension(this);" disabled /><span class="f_pad"> <%=propBean.getMaxUploadLimit("MB")%></span><br />
						<div style="height:24px;">
							<a tabindex="0"><font style='font-size:8pt;color=gray'>첨부파일: <b><%=f.getFileName()%></font></a>
							<input title="파일삭제" type="checkbox" name="fileupdate" id="fileupdate" value="<%=f.getNo()%>" onclick="checkFile(this,1);" onkeypress="checkFile(this,1);">
						<span style="padding-top:3px"><a tabindex="0"><font style='font-size:8pt;color=gray'>왼쪽 체크박스를 선택하시면 이미 첨부된 화일은 삭제됩니다.</font></a></span>
						</div>
<%
		} else {
%>
						<input type="file" name="file1" id="file1" value="" class="input" style="width:330px;" title="첨부파일1" onchange="filterExtension(this);" /><span class="f_pad"> <%=propBean.getMaxUploadLimit("MB")%></span><br />
<%			
		}
	} else {
%>
...
%>					
		...
		<tr>
			<th class="th1"><label for="file2">PDF파일 업로드</label></th>
			<td class="td1">
				<div class="filedv1">
<%	 	
	...
	if (flag.equals("Y")) {
%>
					<input type="file" name="file2" id="file2" value="" class="input" style="width:330px;" title="첨부파일2" onchange="filterExtension(this);" disabled /><span class="f_pad"> <%=propBean.getMaxUploadLimit("MB")%></span><br />
					<div style="height:24px;">
						<a tabindex="0"><font style='font-size:8pt;color=gray'>첨부파일: <b><%=f.getFileName()%></font></a>
						<input title="파일삭제" type="checkbox" name="fileupdate" id="fileupdate" value="<%=f.getNo()%>" onclick="checkFile(this,2);" onkeypress="checkFile(this,2);">
					<span style="padding-top:3px"><a tabindex="0"><font style='font-size:8pt;color=gray'>왼쪽 체크박스를 선택하시면 이미 첨부된 화일은 삭제됩니다.</font></a></span>
					</div>
<%
	} else {
%>
					<input type="file" name="file2" id="file2" value="" class="input" style="width:330px;" title="첨부파일2" onchange="filterExtension(this);" /><span class="f_pad"> <%=propBean.getMaxUploadLimit("MB")%></span><br />
<%			
	}
%>
				</div>				
