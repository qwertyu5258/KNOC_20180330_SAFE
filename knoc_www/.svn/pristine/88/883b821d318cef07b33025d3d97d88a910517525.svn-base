...
<input name="virtualno" type="hidden" value="<%= userSession==null ? "":userSession.getUserVirtualno()%>" />
	...
	<colgroup><col width="110px" /><col width="590px" /></colgroup>
		...
		<tr>
			...
			<td class="td12"><textarea name="HTML_BODY" id="HTML_BODY" cols="95" rows="15" class="area"></textarea></td>
		</tr>
		<% if ( propBean.getUseSecret().equals("1")) { %>
		<tr>
			<th scope="row" class="th1">공개여부</th>
			<td class="td1">
				<label for="openOk"><input type="radio" name="secret" id="openOk" value="0" checked />공개</label> <label for="openNo"><input type="radio" name="secret" id="openNo" value="1" /> 비공개</label>
			</td>
		</tr>
		<% } %>
		...
		<tr>
			...
			<td class="td1">
				<input  alt="공지글" title="공지글" type="checkbox" name="notice" id="notice" value="1" /> 공지
			</td>
		</tr>
		<% } %>
	...
	<div class="boardbtm">
		<div class="btnsec">
			<input type="image" onclick="do_submit(document.writeform);" src="<%=skinRoot %>/images/btn_ok.gif" alt="확인" /></a>
