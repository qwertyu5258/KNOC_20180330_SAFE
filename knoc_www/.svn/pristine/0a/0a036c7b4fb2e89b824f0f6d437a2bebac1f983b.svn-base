...
%>	
			...
			%>
				<%
					...
					String imgStyle = "style=\"margin-left:25px;\"";
					if("ENGAN".equals(propBean.getBoardID())){
						imgClass = "imgbg_h2";
						imgSizeStr = "width=\"116\" height=\"152\"";
						imgStyle = "style=\"margin-left:20px;\"";
					}
				%>
					 <div class="<%=imgClass %>" <%=imgStyle %>><img src="/upload/neoboard/<%=propBean.getBoardID()%>/T<%=af.getNewFileName()%>" alt="" <%=imgSizeStr %> /></div>
				<% 
						 }
					} %>  
			<% } else { %>
			...
			<% } %>
					...
					<dd style="padding:5px 0 0 0; width:180px;">
			<% 
			String flag = "N";
			if(article.getFiles() != null && article.getFiles().size() > 0) { 
				AttachFiles afs = article.getFiles();
			...
			%>
					 ...
					 <% } %>
					<img src="../images/sub02/btn_ebook.gif" alt="E-BOOK view" /></a> &nbsp;<a target="_parent" href="<%=Property.SERVLET_ROOT%>/Download?num=<%=article.getNo()%>&amp;fno=<%=af.getNo()%>&amp;bid=<%=propBean.getBoardID()%>&amp;callback=<%=pageBean.getPageUrl()%>&amp;ses=<%=sessionName%>"><img src="../images/sub02/btn_pdf.gif" alt="PDF download" /></a>
				<% 
							flag = "N";
						 } else {
							 flag = "Y";
			...
			if (flag.equals("Y")) {
			%>
					<div class="btnin"><a href="<%=article.getUrl()%>" target="_blank" title="새창"><%-- 20130205 kgs 임시주석처리 <img src="<%=skinRoot %>/images/btn_ebook.gif" alt="E-BOOK다운받기" /> --%></a> &nbsp;<img src="../images/sub02/btn_pdf.gif" alt="PDF download" />
			<%				
			}
			%>						
						</div>
