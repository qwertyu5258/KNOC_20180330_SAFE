...
public class SangdamArticle extends HttpServlet {
		...
		throws ServletException, IOException
	{
	     ...
	     * 보안약점 조치
	     */
	    int maxUploadSize = p.getMaxUpload();
		...
		String ans_email = EtcUtil.strBlockSpecialTags(EtcUtil.checkNull(mrequest.getParameter("ans_email")));
		...
		String ans_phone = tel1 + "-" + tel2 + "-" + tel3;
		...
		article.setAnsEmail(ans_email);
		article.setAnsPhone(ans_phone);
		...
		/* DB 입력 */
		/* 입력 실행 */
        if(manager.editSangdamArticle(p,article,fileupdate)) {
        	/* Expose update*/
