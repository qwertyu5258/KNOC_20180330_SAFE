...
public class SearchManager {
	...
	public ArrayList<SearchBean> getSearchList(String key_word, int page, int pageSize) {
		...
		query.append(
				"select bno as no, boardid, bno, boardname, subject, url, regdate, grp, ROW_NUMBER() OVER(ORDER BY bno desc) AS PAGE_NO  ");
	...
	public ArrayList<SearchBean> getSearchList2(String key_word, int page, int pageSize) {
		...
		query.append(" where is_delete='N'");
		if (key_word != null && !key_word.equals("")) {
			query.append(" and (subject like '%" + "경영" + "%' or content like '%" + "경영" + "%')");
		}
		// query.append(" order by no desc limit ?, ?");
		query.append(" order by bno desc  ");
		query.append(") a ");
