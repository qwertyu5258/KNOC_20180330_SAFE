...
<% 
	...
	String gNavPage = "팝업존관리";  					// 현제 페이지 제목
	/*'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''*/
%>
<%
	...
	pp.setPageSize("10");
	pp.setPageUrl("");
	ArrayList al = PopupMa.getPopupList(pp.getNPage(), pp.getNPageSize());		
	
	int count = PopupMa.getPopupListCount();
...
<script language="javascript">
	...
	function edit(no) {
         	document.bForm.action = 'popupEdit.jsp';
         	document.bForm.no.value = no;
           	document.bForm.submit();
	}
	function jsWrite() {
		location.href = "popupWrite.jsp";
	}
...
<td valign=top>
	...
	<td valign="top">
		...
		<td valign="top">			
			...
			<tr>
			<td><img src="/SiteControl/images/admin_left_03.gif" width="11" height="11"> 기타 관리 > 팝업존관리</td>
			<td width="500" align="right" style="padding-right:20;">HOME > <span class="style6"> 기타 관리 > 팝업존관리</span></td>
			</tr>
			<tr>
			...
			<input type=button value=' 팝업존등록  ' class=button onClick="jsWrite();">
			</td>
			</tr>        
			</table>	
			<br>		
...
<form name="bForm" method="post" action="popupList.jsp">
<input type=hidden name=page value="<%=nowpage%>">	
<input type="hidden" name="no" value="">	
			<tr height="25"  bgcolor="E7E7E7">
			<td align=center width="80">번호</td>
			<td align=center width="150">팝업이미지</td>
...
<% 
		...
		while(iter.hasNext()){
			PopupBean pb = (PopupBean)iter.next();
%>																																	
			<tr height="25"  bgcolor="#FFFFFF">
			<td align=center><%=seq %></td>
			<td align=center><% if (pb.getNewFilename() != null && !pb.getNewFilename().equals("")) { %><img src="/upload/POPUP/<%=pb.getNewFilename() %>" width="130" height="70" /><% } %></td>			
			<td align=center><a href="javascript:edit('<%= pb.getNo() %>')"><%= pb.getTitle() %></a></td>
			<td align=center><%=StringUtil.getDateType(etcutil.checkNull(pb.getStartDate()),"-") %> ~ <%=StringUtil.getDateType(etcutil.checkNull(pb.getEndDate()),"-") %></td>
...
%>				
		...
		<br>		
<stl:pager pageName="pp" prevPage="prevPage" nextPage="nextPage" prevBlock="prevBlock" nextBlock="nextBlock" pages="pages" firstPage="firstPage" lastPage="lastPage" blockSize="10" totalCount="<%=PopupMa.getPopupListCount()%>" />
