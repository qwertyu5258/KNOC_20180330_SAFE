...
<input name="name" type="hidden" value="<%=article.getName()%>" />	
	...
	<colgroup><col width="110px" /><col width="130px" /><col width="110px" /><col width="130px" /><col width="110px" /><col width="130px" /></colgroup>
		...
		<tr>
			...
			<td class="td1" colspan="5"><input type="text" value="<%=phone1 %>" id="tel1" name="tel1" class="input" style="width:25px;" maxlength="3" /> - <input type="text" value="<%=phone2 %>" id="tel2" name="tel2" class="input" style="width:25px;" maxlength="4" /> - <input type="text" value="<%=phone3 %>" id="tel3" name="tel3" class="input" style="width:25px;" maxlength="4" /></td>
		</tr>
		<tr>
			<th class="th1">공개형태</th>
			<td class="td1" colspan="5">열람ㆍ시청 <input name="viewStatus" type="radio" value="열람ㆍ시청" <% if (article.getViewStatus().equals("열람ㆍ시청")) { %>checked="checked"<% } %> /> 사본ㆍ출력물 <input name="viewStatus" type="radio" value="사본ㆍ출력물" <% if (article.getViewStatus().equals("사본ㆍ출력물")) { %>checked="checked"<% } %> /> 전자파일 <input name="viewStatus" type="radio" value="전자파일" <% if (article.getViewStatus().equals("전자파일")) { %>checked="checked"<% } %> /> 복제ㆍ인화물 <input name="viewStatus" type="radio" value="복제ㆍ인화물" <% if (article.getViewStatus().equals("복제ㆍ인화물")) { %>checked="checked"<% } %> /> 기타 <input name="viewStatus" type="radio" value="기타" <% if (article.getViewStatus().equals("기타")) { %>checked="checked"<% } %> /></td>
		</tr>
		<tr>
			<th class="th1">수령방법</th>
			<td class="td1" colspan="5">직접방문 <input name="kind" type="radio" value="직접방문" <% if (article.getKind().equals("직접방문")) { %>checked="checked"<% } %> /> 우편 <input name="kind" type="radio" value="우편" <% if (article.getKind().equals("우편")) { %>checked="checked"<% } %> />   모사전송 <input name="kind" type="radio" value="모사전송" <% if (article.getKind().equals("모사전송")) { %>checked="checked"<% } %> />   전자우편 <input name="kind" type="radio" value="전자우편" <% if (article.getKind().equals("전자우편")) { %>checked="checked"<% } %> />  기타 <input name="kind" type="radio" value="기타" <% if (article.getKind().equals("기타")) { %>checked="checked"<% } %> />  </td>
		</tr>
		<tr>
			<th class="th1">사용목적</th>
			<td class="td1" colspan="5">학술연구 <input name="object" type="radio" value="학술연구" <% if (article.getObject().equals("학술연구")) { %>checked="checked"<% } %> /> 사업관련 <input name="object" type="radio" value="사업관련" <% if (article.getObject().equals("사업관련")) { %>checked="checked"<% } %> /> 행정감시 <input name="object" type="radio" value="행정감시" <% if (article.getObject().equals("행정감시")) { %>checked="checked"<% } %> /> 쟁송관련 <input name="object" type="radio" value="쟁송관련" <% if (article.getObject().equals("쟁송관련")) { %>checked="checked"<% } %> /> 재산관리 <input name="object" type="radio" value="재산관리" <% if (article.getObject().equals("재산관리")) { %>checked="checked"<% } %> /> 기타 <input name="object" type="radio" value="기타" <% if (article.getObject().equals("기타")) { %>checked="checked"<% } %> /></td>
		</tr>
		<tr>
			<th class="th1">수수료감면</th>
			<td class="td1" colspan="5">해당 <input name="cut" type="radio" value="해당" <% if (article.getCut().equals("해당")) { %>checked="checked"<% } %> /> 해당없음 <input name="cut" type="radio" value="해당없음"  <% if (article.getCut().equals("해당없음")) { %>checked="checked"<% } %>/></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="td1" colspan="5">
				<input type="radio" name="secret" value="0" <% if(article.getSecret().equals("0")) { %>checked="checked"<% } %> />공개 <input type="radio" name="secret" value="1" <% if(article.getSecret().equals("1")) { %>checked="checked"<% } %> /> 비공개
			</td>
		</tr>			
		<% } %>
