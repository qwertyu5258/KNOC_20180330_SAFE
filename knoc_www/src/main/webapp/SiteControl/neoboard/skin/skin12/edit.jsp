...
<input name="name" type="hidden" value="<%=article.getName()%>" />	
	...
	<colgroup><col width="110px" /><col width="590px" /></colgroup>
		...
		<tr>
			...
			<td class="td1">
				<input type="radio" name="secret" value="0" <% if (article.getSecret().equals("0")) { %>checked<% } %> />No <input type="radio" name="secret" value="1" <% if (article.getSecret().equals("1")) { %>checked<% } %> /> Yes
			</td>
		</tr>			
		<% } %>
		<% if(propBean.getUseNotice().equals("1") && Property.isViewable("notice",authBean,propBean,userSession)) { %>
		<tr>
			<th class="th1"><label for="subjtxt">Notice</label></th>
			<td class="td1">
				<input  alt="공지글" title="공지글" type="checkbox" name="notice" id="notice" value="1" <% if (article.getNotice().equals("1")) { %>checked<% } %> /> Notice
			</td>
		</tr>			
		<% } %>
