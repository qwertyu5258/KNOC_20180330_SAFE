...
<script language=javascript>
	...
	function getGovPublicKindList(p_obj){
		location.href = "/SiteControl/neoboard/board_06.jsp?gpk=" + p_obj.value;
	}
	...
	function goWriteFn(){
		location.href = "/SiteControl/neoboard/board_06_edit.jsp?gpk=<%=gpk%>&page=<%=nowpage %>&search_type=<%=search_type %>&search_keyword=<%=search_keyword %>";
	}

	function goModifyFn(p_gov_public_nid){
		location.href = "/SiteControl/neoboard/board_06_edit.jsp?gpk=<%=gpk%>&page=<%=nowpage %>&search_type=<%=search_type %>&search_keyword=<%=search_keyword %>&gov_public_nid=" + p_gov_public_nid;
	}
	function goFileListFn(p_gov_public_nid){
		location.href = "/SiteControl/neoboard/board_06_filelist.jsp?gpk=<%=gpk%>&page=<%=nowpage %>&search_type=<%=search_type %>&search_keyword=<%=search_keyword %>&gov_public_nid=" + p_gov_public_nid;
	}
	...
	<tr>
		...
		<td valign="top">
								...
								<td valign="top">
									...
									<table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="B5B5B5">
										...
										<tr height="25" bgcolor="E7E7E7">
											...
											<td align="center">공표목록</td>
											<td align="center">담당부서</td>
											<td align="center">공개시기</td>
											<td align="center">공개주기</td>
											<td align="center">공표매체</td>
											<td align="center">내용수정</td>
										</tr>
									<%if(count == 0){ %> 
									...
									%>
										<tr height="25" bgcolor="#FFFFFF">
											...
											<td align="center" style="word-break:break-all;">
												<a href="#view" onclick ="goModifyFn('<%=hashMap.get("GOV_PUBLIC_NID") %>')"><input style="cursor:pointer;" type="button" value="수정"></a>
											</td> 
										</tr>
									<%				
												seq--;
									...
									<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="B5B5B5">
										<tr>
											...
											<td bgcolor="#FFFFFF" align="right"><input type="button" value="등록" style="width:80px; padding-top:4px; cursor:pointer;" onclick="goWriteFn();"></td>
										</tr>
									</table>	
			...
			<div style="text-align:center;">
				<form method="post" name="schFm" action="board_06.jsp">
					...
					<select name="search_type" style="vertical-align:middle;">
						...
						<option value="TITLE" <%="TITLE".equals(search_type)?"selected":"" %>>공표목록</option>
