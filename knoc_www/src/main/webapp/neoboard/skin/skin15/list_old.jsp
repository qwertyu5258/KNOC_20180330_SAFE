...
<% } else { %>		
	...
	<td valign="top">
		...
		<tr>
		<td height="380">
		<img src="<%=top_img %>" id="bimg" width="370" height="380" />
		</td>
		</tr>
		</table>
	</td>
...
%>				
		...
		<% if(article.getFiles() != null && article.getFiles().size() > 0) { %>			
			<%  AttachFiles afs = article.getFiles();
					...
					if(af.getMimeType().equals("gif") || af.getMimeType().equals("jpg") || af.getMimeType().equals("jpeg") ) {
			%>
		<img src="/upload/neoboard/<%=propBean.getBoardID()%>/T<%=af.getNewFileName()%>" width="80" height="50" onclick="changeImg('/upload/neoboard/<%=propBean.getBoardID()%>/T<%=af.getNewFileName()%>');" style="cursor:pointer" />
		<% 
					} %>  
			<% } else { %>
				&nbsp;
