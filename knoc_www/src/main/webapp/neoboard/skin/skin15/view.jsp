...
<% if(Property.isViewable("delete",authBean,propBean,userSession,article.getId())) { %>
	<script type="text/javascript">
		function restoreArticle() {
			...
			document.deleteform.action = "<%=Property.SERVLET_ROOT%>/restoreArticle";
...
<form name="deleteform" method="post" action="<%=Property.SERVLET_ROOT%>/DeleteArticle" onsubmit="deleteSubmit(this); return false;">
<input type="hidden" name="bid" value="<%=propBean.getBoardID()%>" />		
...
<% } %>		
	...
	<colgroup><col width="100px" /><col width="260px" /><col width="100px" /><col width="260px" /></colgroup>
		<tr>
			...
			<td align="left" class="board_titletop2" colspan="3"><%=StringUtil.replaceInString(article.getSubject(),"[답변]","<span class=font_12px_20_bold>[답변]</span>")%></td>			
		</tr>
        <tr>
			<td width="50" align="center" bgcolor="#F1F1F1" class="board_titletop3">Write</th>
			<td align="left" class="board_listbott"><%=article.getName()%></td>
			<td width="90" align="center" bgcolor="#F1F1F1" class="board_titletop3">Date</th>
			<td width="200" align="left" class="board_listbott"><span title="<%=article.getRegDateString("yyyy년 MM월 dd일 HH시 mm분 ss초")%>"><%=article.getRegDateString("yyyy-MM-dd")%></span></td>
		</tr>
...
<form name="moveform" method="post" action="<%=Property.SERVLET_ROOT%>/MoveArticle">
<input type="hidden" name="bid" value="<%=propBean.getBoardID()%>" />		
<input type="hidden" name="callback" value="<%=pageBean.getPageUrl()%>" />
<input type="hidden" name="ses" value="<%=sessionName%>" />
<input type="hidden" name="num" value="<%=article.getNo()%>" />
