...
<input name="virtualno" type="hidden" value="<%= userSession==null ? "":userSession.getUserVirtualno()%>" />
	...
	<colgroup><col width="100px" /><col width="260px" /><col width="100px" /><col width="260px" /></colgroup>
		<tr>
			<th scope="row" class="th1"><label for="htmlBody">답변내용</label></th>
			<td class="td1" colspan="3">
			<textarea name="HTML_BODY" id="htmlBody" cols="95" rows="15" class="area"><%=EtcUtil.checkNull(article.getAnswer()) %></textarea>
			</td>
		</tr>
		<tr>
...
%>
		...
		<tr>
			<th scope="row" class="th1"><label for="buse">처리부서</label></th>
			<td class="td1">
			<select id="buse" name="buse">
				<%=kr.co.knoc.division.DivisionListManager.getDivisionSelect(article.getBuse()) %>
			</select></td>
                        <th scope="row" class="th1"><label for="status">처리결과</label></th>
			<td class="td1">
			<select id="status" name="status">
				...
				<option value="보류" <% if (article.getStatus() != null && article.getStatus().equals("보류")) { %>selected<% } %>>보류</option>
				<option value="부서이관" <% if (article.getStatus() != null && article.getStatus().equals("부서이관")) { %>selected<% } %>>부서이관</option>
				<option value="민원이관" <% if (article.getStatus() != null && article.getStatus().equals("민원이관")) { %>selected<% } %>>민원이관</option>
				<option value="접수완료" <% if (article.getStatus() != null && article.getStatus().equals("접수완료")) { %>selected<% } %>>접수완료</option>
			</select>
			</td>
		</tr>
	</table>
