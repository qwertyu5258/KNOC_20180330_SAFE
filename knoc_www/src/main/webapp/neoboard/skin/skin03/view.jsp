...
<%-- 삭제기능 추가 --%>
function PasswdPop2(URL, p_act_type) {		
	window.open("","PasswdPop","width=302px,height=151px,scrollbars=no");
	document.formPop.callback.value = URL;
	document.formPop.act_type.value = p_act_type;
	document.formPop.submit();	
}
//]]>
</script>
...
<% } %>		
	...
	<colgroup><col width="100px" /><col width="240Px" /><col width="100px" /><col width="259px" /></colgroup>
		<tr>
			<th scope="row" class="th1">처리상태</th>
			<td class="td1" colspan="3"><% if (article.getBuse() != null && !article.getBuse().equals("")) { %>처리부서 : <%=DivisionListManager.getDName(article.getBuse()) %>, 처리상태 : <%=EtcUtil.checkNull(article.getStatus()) %> (처리일 : <%=article.getAnsDateString("yyyy-MM-dd") %>) <% } %></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="td1" colspan="3"><%=article.getViewStatus() %></td>
		</tr>
		<tr>
			<th scope="row" class="th1">수령방법</th>
			<td class="td1" colspan="3"><%=article.getKind() %></td>
		</tr>
        <tr>
			<th scope="row" class="th1">사용목적</th>
			<td class="td1" colspan="3"><%=article.getObject() %></td>
		</tr>
        <tr>
			<th scope="row" class="th1">수수료감면</th>
			<td class="td1" colspan="3"><%=article.getCut() %></td>
		</tr>
		<tr>
			<th scope="row" class="th1">수수료감면사유</th>
			<td class="td1" colspan="3"><%=article.getCutContent() %></td>
		</tr>
        <tr>
			<th scope="row" class="th1">제&nbsp;&nbsp;목</th>
			<td class="td1" colspan="3"><%=article.getSubject() %></td>
		</tr>
		<tr>
...
<input type="hidden" name="psize" value="<%=propBean.getListPerPage()%>" />		
		<div class="replwrite">
			...
			<p>
				<label for="passwd">비밀번호 :</label> <input type="password" name="passwd" id="guestpasswd" value="" class="input" maxlength="10" style="width:80px;" />
			</p>
			<% } %>
			<div class="reptxt">
				<div class="repdvleft">
...
<% if (article.getBuse() != null && !article.getBuse().equals("")) { %>		
		...
		<table cellspacing="0" cellpadding="0" class="viewtype" summary="해당게시판에 게시물을 등록합니다">
		<caption>게시물등록</caption>
		<colgroup><col width="100px" /><col width="249px" /><col width="100px" /><col width="250px" /></colgroup>
			<tr>
...
<!-- 답변 끝 -->
	...
	<div class="boardbtm">
		...
		<%if("INFOOPEN".equals(propBean.getBoardID())){ %>
			<% if(Property.isViewable("delete",authBean,propBean,userSession,article.getId())) { %><a href="#" onclick="PasswdPop2('<%=pageBean.getPageUrl()%><%=pageBean.makeQueryString("delete",true)%><%=schBean.makeQueryString("","bid=" + propBean.getBoardID()+"&amp;ses="+sessionName+"&amp;psize="+propBean.getListPerPage(),false)%>', 'remove');"><img src="<%=skinRoot %>/images/btn_del.gif" alt="삭제" /></a><% } %>
		<%}else{ %>
