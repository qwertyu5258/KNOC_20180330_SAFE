...
<input name="name" type="hidden" value="<%=article.getName()%>" />
	...
	<colgroup><col width="110px" /><col width="130px" /><col width="110px" /><col width="130px" /><col width="110px" /><col width="130px" /></colgroup>
		...
		<tr>
			...
			<label for="question">국내대륙붕 <input name="contGubun" id="question" type="radio" value="국내대륙붕" <% if (article.getContGubun().equals("국내대륙붕")) { %>checked="checked"<% } %> /> / </label>
			<label for="notify">석유개발 <input name="contGubun" type="radio" value="석유개발" <% if (article.getContGubun().equals("석유개발")) { %>checked="checked"<% } %> /> / </label> 
			<label for="praise">건설 <input name="contGubun" type="radio" value="건설" <% if (article.getContGubun().equals("건설")) { %>checked="checked"<% } %> /> / </label>
			<label for="suggest">비축사업 <input name="contGubun" type="radio" value="비축사업" <% if (article.getContGubun().equals("비축사업")) { %>checked="checked"<% } %> /> / </label>
			<label for="suggest">시추선사업 <input name="contGubun" type="radio" value="시추선사업" <% if (article.getContGubun().equals("시추선사업")) { %>checked="checked"<% } %> /> / </label>
			<label for="suggest">기타 <input name="contGubun" type="radio" value="기타" <% if (article.getContGubun().equals("기타")) { %>checked="checked"<% } %> /></label>
			</td>
		</tr>

		<% if ( propBean.getUseSecret().equals("1")) { %>
		<tr>
			<th scope="row" class="th1">공개여부</th>
			<td class="td1" colspan="5">
				<label for="openOk"><input type="radio" name="secret" id="openOk" value="0" <% if(article.getSecret().equals("0")) { %>checked="checked"<% } %> />공개 <label for="openNo"><input type="radio" name="secret" id="openNo" value="1" <% if(article.getSecret().equals("1")) { %>checked="checked"<% } %> /> 비공개
			</td>
		</tr>			
		<% } %>
		<tr>
