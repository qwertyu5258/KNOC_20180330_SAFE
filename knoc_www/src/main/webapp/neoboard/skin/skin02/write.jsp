...
<% if(!"SANGDAM".equals(propBean.getBoardID()) && !"CLEAR2".equals(propBean.getBoardID()) ){ %>
		<tr>
			...
			<td class="td1" colspan="5"><label for="question">질문 <input name="contGubun" type="radio" id="question" value="질문" checked="checked" /></label> <label for="notify">불편신고 <input name="contGubun" id="notify" type="radio" value="불편신고" /></label> <label for="praise">직원칭찬 <input name="contGubun" type="radio" id="praise" value="직원칭찬" /></label> <label for="suggest">제안 <input name="contGubun" type="radio" id="suggest" value="제안" /></label> <label for="others">기타 <input name="contGubun" type="radio" id="others" value="기타" /></label></td>
		</tr>
<%} %>
		...
		<tr>
			...
			<td class="td1" colspan="5"><textarea name="HTML_BODY" cols="95" rows="15" id="HTML_BODY" class="area" title="내용입력"></textarea></td>
		</tr>
	...
	<div class="checkAgree">
		<input type="checkbox" id="agree" name="agree" value="Y" class="vc"/> <label for="agree">상기 안내를 확인하였으며 개인정보 수집에 동의함.</label>
	</div>
</form>
	<div class="boardbtm">
		<div class="btnsec">
