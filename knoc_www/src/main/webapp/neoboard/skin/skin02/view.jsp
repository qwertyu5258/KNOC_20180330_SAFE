...
<% } %>		
	...
	<colgroup><col width="13%" /><col width="37%" /><col width="13%" /><col width="37%" /></colgroup>
		<tr>
			<th scope="row" class="th1"><label for="subjtxt">처리부서</label></th>
			<td class="td1"><%=DivisionListManager.getDName(article.getBuse()) %></td>
	                       <th scope="row" class="th1"><label for="subjtxt">처리상태</label></th>
			<td class="td1"><%=EtcUtil.checkNull(article.getStatus()) %></td>
		</tr>
		<% if (article.getSecret().equals("1")) { %>
		<tr>
			<th scope="row" class="th1"><label for="subjtxt">성명</label></th>
			<td class="td1"><%=article.getName() %></td>
                        <th scope="row" class="th1"><label for="subjtxt">등록일</label></th>
			<td class="td1"><span title="<%=article.getRegDateString("yyyy년 MM월 dd일 HH시 mm분 ss초")%>"><%=article.getRegDateString("yyyy-MM-dd")%></span></td>
		</tr>
		<tr>
			<th scope="row" class="th1"><label for="subjtxt">전화번호</label></th>
			<td class="td1"><%=article.getPhone() %></td>
                        <th scope="row" class="th1"><label for="subjtxt">이메일</label></th>
			<td class="td1"><%=article.getEmail() %></td>
		</tr>		
		<% } else { %>
		<tr>
            <th scope="row" class="th1"><label for="subjtxt">등록일</label></th>
			<td class="td1" colspan="3"><span title="<%=article.getRegDateString("yyyy년 MM월 dd일 HH시 mm분 ss초")%>"><%=article.getRegDateString("yyyy-MM-dd")%></span></td>
		</tr>
		<% } %>
		
		<% if ( propBean.getUseSecret().equals("1")) { %>
		<tr>
			<th scope="row" class="th1"><label for="subjtxt">공개형태</label></th>
...
<% if(!"SANGDAM".equals(propBean.getBoardID()) && !"CLEAR2".equals(propBean.getBoardID()) ){ %>
		<tr>
			<th scope="row" class="th1"><label for="contgubun">내용구분</label></th>
			<td class="td1" colspan="3"><%=article.getContGubun() %></td>
		</tr>
<%} %>
		<tr>
			<th scope="row" class="th1"><label for="subject">제목</label></th>
			<td class="td1" colspan="3"><%=StringUtil.replaceInString(article.getSubject(),"[답변]","<span class=font_12px_20_bold>[답변]</span>")%></td>
		</tr>
		<tr>
			<th scope="row" class="th1"><label for="content">내용</label></th>
...
<% if (article.getBuse() != null && !article.getBuse().equals("")) { %>				
		...
		<colgroup><col width="720px" /></colgroup>
			<tr>
				<th scope="row" class="th3"><img src="/images/board/6-1viewte.gif" alt="답변" /></th>
			</tr>
		</table>
...
<!-- 답변 끝 -->	
	...
	<div class="boardbtm">
		...
		<%if("SANGDAM".equals(propBean.getBoardID()) || "SUGEST".equals(propBean.getBoardID()) || "CLEAR2".equals(propBean.getBoardID())){ %>
