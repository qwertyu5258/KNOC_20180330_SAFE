...
<input name="name" type="hidden" value="<%=article.getName()%>" />
	...
	<colgroup><col width="110px" /><col width="590px" /></colgroup>
		...
		<tr>
			...
			<td class="td1">
				<label for="openOk"><input type="radio" name="secret" id="openOk" value="0" <% if (article.getSecret().equals("0")) { %>checked<% } %> />공개</label> <label for="openNo"><input type="radio" name="secret" id="openNo" value="1" <% if (article.getSecret().equals("1")) { %>checked<% } %> /> 비공개</label>
			</td>
		</tr>			
		<% } %>
