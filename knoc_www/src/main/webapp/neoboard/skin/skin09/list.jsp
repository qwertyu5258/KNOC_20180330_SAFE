...
%>	
			...
			%>
					 ...
					 <% } %>
				<%if("PETRO".equals(propBean.getBoardID())){ %>
					<img src="<%=skinRoot %>/images/btn_view.gif" alt="보기" /></a> &nbsp;<a target="_parent" href="<%=Property.SERVLET_ROOT%>/Download?num=<%=article.getNo()%>&amp;fno=<%=af.getNo()%>&amp;bid=<%=propBean.getBoardID()%>&amp;callback=<%=pageBean.getPageUrl()%>&amp;ses=<%=sessionName%>"><img src="<%=skinRoot %>/images/btn_download.gif" alt="download" /></a>					 
				<%}else{ %>
					<img src="<%=skinRoot %>/images/btn_ebook.gif" alt="E-BOOK다운받기" /></a> &nbsp;<a target="_parent" href="<%=Property.SERVLET_ROOT%>/Download?num=<%=article.getNo()%>&amp;fno=<%=af.getNo()%>&amp;bid=<%=propBean.getBoardID()%>&amp;callback=<%=pageBean.getPageUrl()%>&amp;ses=<%=sessionName%>"><img src="<%=skinRoot %>/images/btn_pdf.gif" alt="PDF다운받기" /></a>
				<%} %>
				<% 
							flag = "N";
						 } else {
			...
			%>
				<%if("PETRO".equals(propBean.getBoardID())){ %>
					<div class="btnin"><a href="<%=article.getUrl()%>" target="_blank" title="새창"><img src="<%=skinRoot %>/images/btn_view.gif" alt="보기" /></a>
				<%}else{ %>
					<div class="btnin"><a href="<%=article.getUrl()%>" target="_blank" title="새창"><img src="<%=skinRoot %>/images/btn_ebook.gif" alt="E-BOOK다운받기" /></a> &nbsp;
				<%} %>
			<%				
			}
			%>						
