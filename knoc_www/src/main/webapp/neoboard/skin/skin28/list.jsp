...
%>	
		<tr>
			...
			<%=articleN.getSubject(30)%>			
			<!-- New 아이콘 -->		
			<% if ( propBean.getUseNew().equals("1")) { %><span ><%=articleN.makeNew("<img alt='new' src='"+skinRoot+"/images/btn_new.gif'>",propBean.getNewLimit()*24)%></span><% } %>
			<!-- Cool 아이콘 -->
			<% if ( propBean.getUseCool().equals("1") && (articleN.getHitcnt() >= propBean.getCoolLimit())) { %><span ><img alt='new' src='<%=skinRoot %>/images/btn_cool.gif'></span><% } %>
			<!-- 댓글 개수 -->
...
%>	
		<tr>
			...
			<td class="subj"><%=article.getSubject(30).replaceAll("&#38","&#38;")%></td>
