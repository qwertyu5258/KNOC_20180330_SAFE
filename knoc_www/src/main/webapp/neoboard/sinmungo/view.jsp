...
<form action="list.jsp" method="post" name="form01" id="form01">
	...
	<input type="hidden" name="searchValue" value="<%=etcutil.strBlockSpecialTags(etcutil.checkNull(request.getParameter("searchValue"))) %>">
				...
				<table cellspacing="0" cellpadding="0" class="viewtype" summary="GREAT WAY 게시판의 상세정보를 입력합니다.">
					...
					<tr>
						<th scope="row" class="th1"><label for="subject">신고유형</label></th>				
						<td class="cnts1" colspan="2">
						<%
							if(etcutil.checkNullInt(dataMap.get("TYPE").toString(), 0) ==  1){
								out.print("인사 청탁행위 신고");
							}else if(etcutil.checkNullInt(dataMap.get("TYPE").toString(), 0) ==  2){
								out.print("인사 알선행위 신고");
							}else if(etcutil.checkNullInt(dataMap.get("TYPE").toString(), 0) ==  3){
