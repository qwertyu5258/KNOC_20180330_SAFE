...
<!--
	...
	function goUserChange(){
		document.getElementById("searchform").action = "/sinmungo/password.jsp";
		document.getElementById("searchform").submit();
...
-->			
				...
				%>					
					<tr>
						...
						<td>
						<%
						if(etcutil.checkNullInt(articleMap.get("TYPE").toString(), 0) ==  1){
							out.print("인사 청탁행위 신고");
						}else if(etcutil.checkNullInt(articleMap.get("TYPE").toString(), 0) ==  2){
							out.print("인사 알선행위 신고");
						}else if(etcutil.checkNullInt(articleMap.get("TYPE").toString(), 0) ==  3){
							out.print("기타 인사관련 비리행위 신고");
						}
						 %>
						</td>
